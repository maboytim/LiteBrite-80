...
;
; TRS-80 Model 4 version
;
; The TRS-80 M4 has display and keyboard optionally mapped at F800h and F400h
; respectively.  However they are enabled/disabled together so it's not possible
; to enable only the display but not the keyboard.  The Z80 can run from the
; display memory but not the keyboard.  The display and keyboard could just be
; disabled but including the display memory is more interesting - this requires a 
; 'gratuitous move' for the keyboard block (see the M1/M3 version for explanation
; of gratuitous move).
;
...
filldta ld      a,'*'
        ...
        di
        ld      a,86h           ;enable display and keyboard
        out     (84h),a   
...
deftxt  defb    " Say hi to LB-80-M4! ",0
...
pxon    ld      e,a
        ld      a,h
        cp      0F8h            ;compare dest with display ram start
        ld      a,e
        jr      nc,pxondsp
        ld      a,h
        cp      0F4h            ;compare dest with keyboard start
        ld      a,e
        jr      nc,gratu        ;do gratuitous move if not ram
pxondsp
